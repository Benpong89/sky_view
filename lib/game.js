...
function drawBossHealth() {
  ...
  ctx.lineWidth = "6";
  ctx.strokeStyle = "red";
  ctx.rect(110, 15, bossHealth, 20);
  ctx.stroke();
  ctx.font = "12px Arial";
  ctx.fillText("Boss Health", 180, 50);
}
...
function scoreTotal() {
  ...
  ctx.fillText("Health Bar", 350, 50);
  ctx.beginPath();
  ctx.lineWidth = "6";
  ctx.strokeStyle = "green";
  ctx.rect(330, 15, health, 20);
  ctx.stroke();
  ...
  firebase
    ...
    .then(response => {
      ...
      for (k in userProfiles) {
        scorePosts[k] = userProfiles[k].userScore;
      }

      let props = Object.keys(scorePosts).map(function(key) {
      ...
      for (users of topFive) {
        $("#highscores").append(`<li>${users.key}: ${users.value}</li>`);
      }
    });
}
